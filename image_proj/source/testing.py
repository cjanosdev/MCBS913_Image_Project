...
def static_rag(prompt_convo: dict):
    ...
    conversation.append(prompt_convo)

    print(f"\n\nPrinting the conversation: {conversation}\n\n")
    #print(f"\n\nConversation started up with prompt1.json which uses system to provide llama-3.2-11B-Vision-Instruct a lot of biology fasts: {conversation}\n\n")
    response = get_inference(auth=AUTH_KEY, model=MODEL_LLAMA_VIS, conversation=prompt, streaming=False)
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                ...
                #print(f"\n\nConversation after initial preload with biology info and then first user input to convo that results in deepthought error here is the convo: {conversation}\n\n")
